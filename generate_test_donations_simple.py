...
Simplified version with fixed prices for AAPL and BTC
"""
import csv
import random
from datetime import datetime, timedelta

# Sample charity names
charity_names = [
    "American Red Cross",
...
# Item categories and specific items for donations
item_categories = [
...
AAPL_PRICE = 185  # Reasonable AAPL stock price
BTC_PRICE = 45000  # Reasonable Bitcoin price
...
def generate_items_donation():
    ...
    for _ in range(num_items):
        ...
        # Reasonable value ranges by category
        if category == "Electronics":
            value = random.randint(50, 800)
...
def generate_stock_donation():
    """Generate an AAPL stock donation with fixed price"""
    shares = random.choice([1, 2, 5, 10, 15, 20, 25, 30, 50, 100])
    amount = shares * AAPL_PRICE
    ...
    return {
        ...
        "stock_symbol": "AAPL",
        "shares": shares,
        "notes": f"Donated {shares} shares of AAPL at ${AAPL_PRICE}/share"
    }

def generate_crypto_donation():
    """Generate a Bitcoin donation with fixed price"""
    # Generate reasonable BTC amounts (0.001 to 0.5 BTC)
    btc_amount = round(random.choice([0.001, 0.002, 0.005, 0.01, 0.02, 0.05, 0.1, 0.2, 0.3, 0.5]), 4)
    amount = round(btc_amount * BTC_PRICE, 2)
    ...
    return {
        ...
        "notes": f"Donated {btc_amount} BTC (valued at ${BTC_PRICE:,}/BTC)"
    }

# Generate 100 donations
donations = []

# Distribution: 55 cash, 25 items, 10 miles, 7 stock, 3 crypto
type_distribution = (
    ["cash"] * 55 +
    ["items"] * 25 +
    ["miles"] * 10 +
...
# Write to CSV
with open('test_donations_simple.csv', 'w', newline='', encoding='utf-8') as f:
    fieldnames = [
...
for dtype, stats in sorted(by_type.items()):
    ...
    print(f"  {dtype}: {stats['count']} donations, ${stats['total']:,.2f} total (avg: ${avg:.2f})")

print(f"\nFixed prices used:")
print(f"  AAPL: ${AAPL_PRICE}/share")
print(f"  BTC: ${BTC_PRICE:,}/BTC")
print(f"\nSaved to test_donations_simple.csv")
print("\nNote: You'll need to map charity_name to actual charity IDs from your database when importing")
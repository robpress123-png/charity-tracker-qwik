...
- ✅ **Ultrawide Monitor Support**: Enabled page scrolling for better display on ultrawide screens
  - Changed body from fixed height to min-height with scrolling
  - Removed overflow restrictions on main content areas
  - Sections now use min-height instead of max-height calculations
...
```bash
npm run bump:patch  # 2.11.20 → 2.11.21
npm run bump:minor  # 2.11.20 → 2.12.0
npm run bump:major  # 2.11.20 → 3.0.0
```
...
- v2.11.20: Fixed ultrawide monitor display with page scrolling

## File Structure
```
charity-tracker-qwik/
├── dist/
│   ├── index.html             # v2.11.20 - Simplified landing with navigation buttons
│   ├── dashboard.html          # v2.11.20 - Enhanced stats display with scrolling support
│   ├── admin-dashboard.html    # v2.11.20 - Admin console with proper name display
│   ├── admin-login.html       # v2.11.20 - Secure admin login
│   └── [other pages]
